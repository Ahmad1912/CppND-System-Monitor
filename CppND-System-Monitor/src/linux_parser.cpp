...
float LinuxParser::CpuUtilization(int pid) { 
  ...
  float total_time = sum/sysconf(_SC_CLK_TCK);
  float seconds = (float)LinuxParser::UpTime() - (UTime / sysconf(_SC_CLK_TCK));*/

  float total_time = ActiveJiffies(pid);
  float seconds = UpTime(pid);
  
  return float(1.0*(total_time/seconds)); }
...
float LinuxParser::UpTime(int pid) { 
  ...
  return (float)LinuxParser::UpTime() - (time / sysconf(_SC_CLK_TCK)) ;
  //return time;
